...
// 주문(매수)
async function order_bid(market, volume, price, ord_type) {
    //market: KRW-BTC
    const url = 'https://api.upbit.com/v1/orders'
    let qs = {market:market, side:'bid', volume:volume, price:price, ord_type:ord_type}
    ...
    let result; 
    try {
        result = await request(url, qs, token, 'POST')
    } catch(e) {
        result.data = null
        if (e.error.error) {
            result.message = e.error.error.message
...
// 주문(매도)
async function order_ask(market, volume, price, ord_type) {
    //market: KRW-BTC
    const url = 'https://api.upbit.com/v1/orders'
    let qs = {market:market, side:'ask', volume:volume, price:price, ord_type:ord_type}
...
async function notice_info() {
    const url = 'https://project-team.upbit.com/api/v1/disclosure?region=kr&time='+(new Date()).getTime()
    let result = await request(url)
    return result
}
...
// 공시정보
async function proxy_test() {
    const url = 'https://www.findip.kr/'
...
Upbit.prototype.notice_info = notice_info
Upbit.prototype.proxy_test = proxy_test
